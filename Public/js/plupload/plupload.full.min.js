...
;(function($, o) {
	...
	function renderUI(id, target) {
		...
		target.contents().each(function(i, node) {
                ...
                $('ul.plupload_filelist', target).prepend('<li class="J-add-btn" id="' + id + '_browse"></li>')
                
                /*
		target.prepend(
			'<div class="plupload_foot">' +
                            '<div class="plupload_buttons">' +
                                '<a href="#" class="btn btn-info" id="' + id + '_browse"><i class="ace-icon fa fa-folder-open"></i>' + _('选择图片') + '</a>' +
                                '<a href="#" class="btn btn-success"><i class="ace-icon fa fa-cloud-upload"></i>' + _('开始上传') + '</a>' +
                            '</div>' +
		...
		if (settings) {
			this.each(function() {
				...
				if (!id) {
                                    id = plupload.guid();
                                    target.attr('id', id);
                                    target.find(".plupload_filelist").attr('id', id+"_ul");
				}
				...
				function handleStatus(file) {
					...
					if (file.status == plupload.DONE) {
                                            ...
                                            $('#' + file.id).removeClass("newfile").addClass("J-preview");
					}
					...
					//var icon = $('#' + file.id).attr('class', actionClass).find('a').css('display', 'block');
                                        var icon = $('#' + file.id).addClass(actionClass);
					if (file.hint) {
				...
				function updateList(up, files) {
                                    var ul = $("#"+settings.container+"_ul");
                                    
                                    if((uploader.files.length + ul.find("li.J-preview").length) > settings.max_file_count){
                                        //删除队列
                                        plupload.each(files, function(file) {
                                            uploader.removeFile(file.id);
                                        });
                                        alert("您只能上传"+(settings.max_file_count-(ul.find("li.J-preview").length+uploader.files.length))+"张图片");
                                        
                                        return false;
                                    }
                                    if((uploader.files.length + ul.find("li.J-preview").length) == settings.max_file_count){
                                        ul.find(".J-add-btn").hide();
                                        uploader.disableBrowse(true);
                                        console.log(uploader.files.length + ul.find("li.J-preview").length);
                                    }
					var fileList = $('ul.plupload_filelist', target), inputCount = 0, inputHTML;
                                        fileList.find(".newfile").remove();
						...
						if (file.status == plupload.DONE) {
											...
											if(file.type=='image/vnd.microsoft.icon' || file.type=='image/svg+xml') {
												fileList.find(".J-add-btn").after(
													'<li id="' + file.id + '" class="ui-state-default newfile">' +
													'<div class="cut"><img src="" class="imgsrc"></div><i class="del"></i><div class="loading-img open"><i class="loading-icon"></i><span class="num">0</span></div>' +
													'<input type="hidden" class="typename" name="'+settings.typename+'" />' +
													'</li>'
												);

												$('#' + file.id + ' .del').click(function(e) {
													$('#' + file.id).remove();
													uploader.removeFile(file);
													if((uploader.files.length + fileList.find("li.J-preview").length) < settings.max_file_count){
														ul.find(".J-add-btn").show();
														uploader.disableBrowse(false);
														if(!$(this).find("input").length)       uploader.refresh()
													}
													e.preventDefault();
												});
											}else{
												...
												preloader.onload = function() {
													...
													fileList.find(".J-add-btn").after(
														'<li id="' + file.id + '" class="ui-state-default newfile">' +
														'<div class="cut"><img src="'+preloader.getAsDataURL()+'"></div><i class="del"></i><div class="loading-img open"><i class="loading-icon"></i><span class="num">0</span></div>' +
														'<input type="hidden" class="typename" name="'+settings.typename+'" />' +
														'</li>'
													);
	...
	// Rename file and glue extension back on
					...
					//$('#' + id + '_container').attr('title', 'Using runtime: ' + res.runtime);
                                        var ul = $('ul.plupload_filelist', target)
                                        if((uploader.files.length + ul.find("li.J-preview").length) == settings.max_file_count){
                                            ul.find(".J-add-btn").hide();
                                            uploader.disableBrowse(true);
                                            //console.log(uploader.files.length + ul.find("li.J-preview").length);
                                        }
                                        ul.find("li.J-preview").on("click",".del",function(){
                                            $(this).closest("li").remove();
                                            if(ul.find("li.J-preview").length < settings.max_file_count){
                                                ul.find(".J-add-btn").show();
                                                uploader.disableBrowse(false);
                                                uploader.refresh()
                                            }
					...
					$('a.btn-success', target).click(function(e) {
				...
				uploader.bind('FileUploaded', function(up, file, res) {
                                    handleStatus(file);
                                    if(res.status == 200){
                                        var percentBox = $("#"+file.id);
                                        percentBox.find(".loading-img").remove();
                                        var data = eval('(' + res.response + ')');
                                        if(data.status==1){
											console.log(percentBox.find(".imgsrc"))
											percentBox.find(".imgsrc").attr('src',data.data.img);
                                            percentBox.find(".typename").val(data.data.img);
                                            //删除已上传序列
                                            uploader.removeFile(file.id);
                                        }else{
                                            alert("上传失败,请重新尝试!");
				...
				uploader.bind("UploadProgress", function(up, file) {
                                    ...
                                    var percentBox = $("#"+file.id);
                                    percentBox.find("span.num").html(file.percent);
                                    if(file.percent == 100){
                                        percentBox.find(".loading-icon").remove();
                                        percentBox.find("span.num").html('即将完成');
                                    }
